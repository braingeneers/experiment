...
downloadDirectoryFroms3(BUCKET_NAME,args.uuid)
...
batch_metadata = {
  ...
  #  "issue": "https://github.com/braingeneers/internal/issues/{}".format(args.issue),
    "notes": "\n".join([open(f).read() for f in glob.glob("{}/*.txt".format(args.uuid))])
}
...
for f in glob.glob("{}/*.json".format(args.uuid)):
...
for original_path in sorted(glob.glob("{}".format(args.uuid))):
