...
#    print(UUID)
if UUID[11]=='e':
    path='/public/groups/braingeneers/ephys/'+UUID

if UUID[11]=='i': 
    path='/public/groups/braingeneers/imaging/'+UUID  

if UUID[11]=='f': 
    path='/public/groups/braingeneers/fluidics/'+UUID  

files = get_list_of_files(path)

correct_files=[]
for file in files:
    correct_files.append(file.split('braingeneers/')[1])
...
s3_client = boto3.client('s3', endpoint_url=os.environ['ENDPOINT_URL'])
s3_client.delete_object(Bucket='braingeneers-inbox', Key=UUID+'.zip')
